...
public class CsvUtil {
	...
	public static <T> List<T> read(InputStream is, String charset, Class<T> clazz) {
		try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is))) {
			...
			Constructor<?>[] constructors = new Constructor[columnNames.length];
				...
				for (int i = 0; i < fields.length; i++) {
					if (columnValues[i] != null && columnValues[i].length() > 0) {
						if (constructors[i] != null) {
							fields[i].set(obj, constructors[i].newInstance(columnValues[i]));
						} else {
							fields[i].set(obj, sdf.parse(columnValues[i]));
						}
					}
				}
				results.add(obj);
